-- Create fee_reminders table to track reminder history
CREATE TABLE IF NOT EXISTS fee_reminders (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  student_id UUID NOT NULL REFERENCES students(id) ON DELETE CASCADE,
  receipt_id UUID NOT NULL REFERENCES fee_receipts(id) ON DELETE CASCADE,
  reminder_type TEXT NOT NULL DEFAULT 'overdue_payment', -- 'overdue_payment', 'upcoming_due', etc.
...
-- Create function to identify overdue payments
-- grace_period_days: Number of days after payment_date to consider overdue
...
💰 *Outstanding Amount:* PKR ' || remaining_due_param::TEXT || '
